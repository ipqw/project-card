...
export const ProjectCard = observer((props: IProps) => {
    ...
    return(
        <ProjectWrapper>
            <Image src={image.src} />
            <div>
                <ProjectDate>{project?.createdAt.toLocaleDateString("en-US")}</ProjectDate>
                <ProjectName style={{color: color}}>{project?.name}</ProjectName>
                <ProjectAuthors style={{color: color}}>{authors.map((elem: any) => elem.name).join(', ')}</ProjectAuthors>
                <ProjectDescription style={{color: color}}>{project?.description}</ProjectDescription>
